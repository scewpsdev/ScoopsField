...
layout (set = 2, binding = 1) uniform sampler2D s_sunColor;
layout (set = 2, binding = 2) uniform samplerCube s_environment;
...
vec3 lighting(vec3 position)
{
	...
	for (int i = 0; i < numPointLights; i++)
	{
		...
		radiance += L(pointLightColors[i].rgb, distanceSq, 0.1);
	}

	radiance += textureLod(s_environment, vec3(0, 1, 0), 12).rgb;

	radiance += texture(s_sunColor, vec2(0.0)).rgb * max(dot(-sunDirection.xyz, vec3(0, 1, 0)), 0);
...
void main()
{
	...
	vec3 radiance = color.rgb / PI * (emissive + lighting(v_position));
