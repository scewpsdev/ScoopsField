...
void getCloudParams(float cloudType, out float minHeight, out float maxHeight, out float densityMultiplier, out float structureErosionMultiplier, out float detailStrengthMultiplier, out float heightGradientType)
{
	...
	detailStrengthMultiplier = interpolate3(0.05, 0.07, 0.03, cloudType);
...
float getCloudDensity2(vec3 p, float height, int lod)
{
	...
	if (cloud > 0)
	{
		...
		if (cloud > 0 && detailStrength > 0)
		{
			...
			float detailErosion = detailNoise.r * 0.5 + detailNoise.g * 0.25 + detailNoise.b * 0.25;
			//detailErosion = heightFraction < 0.1 /*|| height > 0.5 * maxCloudHeight*/ ? 1 - detailErosion : detailErosion;
			cloud = remap(cloud, detailErosion * detailStrength * 0.5, 1);
			cloud = max(cloud, 0);
...
vec4 clouds(vec3 origin, vec3 dir, vec3 lightDir, float noise, int lod, int numSamples)
{
	...
	for (int i = 0; i < numSamples; i++)
	{
		...
		if (density > 0)
		{
			...
			float ambient = 0.006;
			float powder = 1 - exp(-density * 10); //(1 - exp(-density * 0.5)) * 1;
			float shadow = beer * powder * 20;
	...
	vec4 aerial = calculateAerial(origin - vec3(0, planetRadius, 0), dir, dist * 1, lightDir);
