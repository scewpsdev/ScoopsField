...
#define GBUFFER_COLOR_ATTACHMENTS 4
	...
	depthAttachment.format = SDL_GPU_TEXTUREFORMAT_D24_UNORM_S8_UINT;
	depthAttachment.loadOp = SDL_GPU_LOADOP_CLEAR;
	depthAttachment.storeOp = SDL_GPU_STOREOP_STORE;
	depthAttachment.clearDepth = 0;
	depthAttachment.stencilLoadOp = SDL_GPU_LOADOP_CLEAR;
	depthAttachment.stencilStoreOp = SDL_GPU_STOREOP_STORE;
...
static RenderTarget* CreateHDRTarget(int width, int height)
{
	...
	hdrDepthInfo.format = SDL_GPU_TEXTUREFORMAT_D24_UNORM_S8_UINT;
	hdrDepthInfo.loadOp = SDL_GPU_LOADOP_LOAD;
	hdrDepthInfo.storeOp = SDL_GPU_STOREOP_STORE;
	hdrDepthInfo.clearDepth = 1;
	hdrDepthInfo.stencilLoadOp = SDL_GPU_LOADOP_LOAD;
	hdrDepthInfo.stencilStoreOp = SDL_GPU_STOREOP_STORE;
	hdrDepthInfo.clearStencil = 0;
...
static SDL_GPUTexture* CreateNormalMips(int width, int height)
{
	...
	targetInfo.usage = SDL_GPU_TEXTUREUSAGE_COLOR_TARGET | SDL_GPU_TEXTUREUSAGE_COMPUTE_STORAGE_WRITE | SDL_GPU_TEXTUREUSAGE_SAMPLER;
	targetInfo.width = width;
	targetInfo.height = height;
	targetInfo.layer_count_or_depth = 1;
	targetInfo.num_levels = SSAO_STEPS;
...
static GraphicsPipeline* CreatePortalMaskPipeline(Renderer* renderer)
{
	...
	pipelineInfo.stencilPassOp = SDL_GPU_STENCILOP_REPLACE;
	pipelineInfo.stencilCompareMask = 0xFF;
	pipelineInfo.stencilWriteMask = 0xFF;

	for (int i = 0; i < pipelineInfo.numColorTargets; i++)
		CreateBlendStateNoColorWrite(&pipelineInfo.colorTargets[i].blend_state);

	//pipelineInfo.depthWrite = false;
	return CreateGraphicsPipeline(&pipelineInfo);
}

static GraphicsPipeline* CreatePortalDepthResetPipeline(Renderer* renderer)
{
	GraphicsPipelineInfo pipelineInfo = CreateGraphicsPipelineInfo(SDL_GPU_PRIMITIVETYPE_TRIANGLELIST, SDL_GPU_CULLMODE_BACK, renderer->portalDepthResetShader, renderer->gbuffer, NUM_MESH_BUFFER_LAYOUTS, renderer->meshLayout);
	...
	pipelineInfo.stencilPassOp = SDL_GPU_STENCILOP_KEEP;
	pipelineInfo.stencilCompareMask = 0xFF;
	pipelineInfo.stencilWriteMask = 0xFF;
...
static GraphicsPipeline* CreateDepthDownsamplePipeline(Renderer* renderer)
{
	...
	GraphicsPipelineInfo pipelineInfo = CreateGraphicsPipelineInfo(SDL_GPU_PRIMITIVETYPE_TRIANGLELIST, SDL_GPU_CULLMODE_BACK, renderer->depthDownsampleShader, 1, &colorFormat, true, SDL_GPU_TEXTUREFORMAT_D24_UNORM, 1, &renderer->screenQuad.vertexBuffer->layout);
...
static void SubmitMesh(Renderer* renderer,
	MeshDrawData* mesh,
	const mat4& projection, const mat4& view, const mat4& pv, vec3 cameraPosition, vec3 sunDirection, bool viewSpaceBuffer, vec4 clippingPlane,
...
void InitRenderer(Renderer* renderer, int width, int height, SDL_GPUCommandBuffer* cmdBuffer)
{
	...
	renderer->portalMaskShader = LoadGraphicsShader("res/shaders/portal/portal_mask.vert.bin", "res/shaders/portal/portal_mask.frag.bin");
	renderer->portalDepthResetShader = LoadGraphicsShader("res/shaders/portal/portal_depth_reset.vert.bin", "res/shaders/portal/portal_depth_reset.frag.bin");
	...
	renderer->reconstructNormalsShader = LoadComputeShader("res/shaders/postprocessing/reconstruct_normals.comp.bin");
	...
	renderer->specularConvoluteShader = LoadComputeShader("res/shaders/lighting/specular_convolute.comp.bin");
	...
	renderer->portalMaskPipeline = CreatePortalMaskPipeline(renderer);
	renderer->portalDepthResetPipeline = CreatePortalDepthResetPipeline(renderer);
	renderer->geometryStencilPipeline = CreateGeometryStencilPipeline(renderer);
	...
	renderer->weather.cloudCoverage = 0.5f;
...
void RenderPortal(Renderer* renderer, Portal* portal)
{
	PortalDrawData drawData = {};
	drawData.portalID = renderer->portals.size + 1;
	drawData.transform = ModelMatrix((Entity*)portal);
	drawData.destinationTransform = ModelMatrix((Entity*)portal->destination);
	drawData.portalView = drawData.transform * mat4::Rotate(vec3::Up, PI) * drawData.destinationTransform.inverted();

	Mesh* mesh = &portal->model->meshes[0];
	...
	data.transform = drawData.transform;

	data.flags = 0;

	drawData.drawData = data;

	renderer->portals.add(drawData);
}
	...
	SDL_GPURenderPass* renderPass, SDL_GPUCommandBuffer* cmdBuffer)
{
	...
	if (mesh->skeleton)
	{
		...
		uniforms.clippingPlane = clippingPlane;
...
static void AmbientOcclusion(Renderer* renderer, mat4 projection, float fov, float near)
{
	...
	// reconstruct normals
	{
		GPU_TIMER("Reconstruct normals");

		SDL_GPUStorageTextureReadWriteBinding targetBinding = {};
		targetBinding.texture = renderer->normalMips;
		targetBinding.mip_level = 0;
		...
		SDL_BindGPUComputePipeline(computePass, renderer->reconstructNormalsShader->compute);

		SDL_PushGPUComputeUniformData(cmdBuffer, 0, &projection, sizeof(projection));

		SDL_GPUTextureSamplerBinding textureBinding = {};
		textureBinding.texture = renderer->hdrTarget->depthAttachment;
		textureBinding.sampler = renderer->samplers[TEXTURE_SAMPLER_DEFAULT];

		SDL_BindGPUComputeSamplers(computePass, 0, &textureBinding, 1);

		SDL_DispatchGPUCompute(computePass, (renderer->width + 31) / 32, (renderer->height + 31) / 32, 1);

		SDL_EndGPUComputePass(computePass);
	}

	// downsample depth & normals
	{
		GPU_TIMER("Downsample Depth & Normals");
		...
		for (int i = 1; i < SSAO_STEPS; i++)
		{
			...
			colorTarget.texture = renderer->normalMips;
			colorTarget.mip_level = i;
			...
			textures[0] = renderer->depthMips;
			textures[1] = renderer->normalMips;
...
void RendererShow(Renderer* renderer, vec3 cameraPosition, quat cameraRotation, float near, float fov, float aspect, mat4 projection, mat4 view, mat4 pv, vec4 frustumPlanes[6], vec3 sunDirection, SDL_GPUTexture* swapchain, SDL_GPUCommandBuffer* cmdBuffer)
{
	...
	// lighting pass
	{
		SDL_GPUCopyPass* copyPass = SDL_BeginGPUCopyPass(cmdBuffer);
		{
			...
			src.texture = renderer->gbuffer->depthAttachment;

			SDL_GPUTextureLocation dst = {};
			dst.texture = renderer->hdrTarget->depthAttachment;

			SDL_CopyGPUTextureToTexture(copyPass, &src, &dst, renderer->width, renderer->height, 1, false);
		}
